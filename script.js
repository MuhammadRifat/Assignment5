// For getting meals after search
function getMeal(){
    const mealName = document.getElementById('input-meal').value;
    fetch('https://www.themealdb.com/api/json/v1/1/search.php?s='+ mealName +'')
    .then(response => response.json())
    .then(data => {
        if(data.meals === null){        //checking meals array is empty or not
...
// For displaying all meals after search
...
// For getting all ingredients after select any meal
...
// For clearing all existing meal
function clearAllMeals(){
    document.getElementById("container").innerText = "";
}

// For clearing all existing ingredients
